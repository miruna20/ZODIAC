...
from utils.util import category_2_to_label
...
class SpinesDataset(torch.utils.data.Dataset):
	def __init__(self, root, filelist, transform, read_file=read_file,
				in_memory=False, take: int = -1):
		super(SpinesDataset, self).__init__()
		self.root = root
		self.filelist = filelist
		self.transform = transform
		self.in_memory = in_memory
	...
	def load_filenames(self):
		...
		for line in lines:
			filename = line.split()[0]
			if "sub-verse" or "Phantom" in filename:
				label = category_2_to_label["spines"]
			elif "sacrum" in filename:
				label = category_2_to_label["sacrum"]
			else:
				raise ValueError("I don't know the label of this data")
			filenames.append(filename)
			labels.append(torch.tensor(label))
