#!/bin/bash
# TP-ODIAC inference — single seed (42), quick run on Balgrist + phantom.
# TP-ODIAC is the supervised, trained-with-pairs variant (its own prior + VAE).
# Output: results/inference/tpodiac/<dataset>/<scan>/seed_42.obj (directly evaluatable)
#
# Run from anywhere: bash scripts/inference_TPODIAC/run_singleseed.sh [GPU_ID]
...
export RESULTS_OUT
VARIANT="tpodiac"; export VARIANT

CKPT="checkpoints/tp-odiac/diffusion/df_steps-latest.pth"
VAE_CKPT="checkpoints/tp-odiac/vae/vae_steps-latest.pth"
DF_CFG="configs/inference/octfusion_tpodiac.yaml"
...
echo "=== TP-ODIAC single-seed complete: results/inference/${VARIANT}/<dataset>/<scan>/seed_42.obj ==="
