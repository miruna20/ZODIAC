...
class OctFusionModel(BaseModel):
    ...
    def batch_to_cuda(self, batch):
        ...
        if self.load_partial:
            points = [pts.cuda(non_blocking=True) for pts in batch['partial_points']]
            octrees = [points2octree(pts) for pts in points]
            octree = ocnn.octree.merge_octrees(octrees)
            octree.construct_all_neigh()
            batch['octree_in_partial'] = octree
    ...
    def set_input(self, input=None):
        self.batch_to_cuda(input) # the pointcloud is converted to octree here
        if self.load_pointcloud:
            self.octree_in = input['octree_in']
        if self.load_partial:
            self.octree_in_partial = input['octree_in_partial']
    ...
    def inference(self, save_folder = "results_vae"):
        ...
        pointcloud = trimesh.PointCloud(vertices=self.batch['points'][0].cpu().points.numpy()) # these points are from pointcloud.npz, so the points that are sampled from the mesh in the preprocessing, not the points of the SDF values.
        pointcloud.export(os.path.join(save_dir, 'input.ply'))

    @torch.no_grad()
    def inference_shape_completion(self, save_folder = "results_vae"):
        self.autoencoder.eval()
        output = self.autoencoder.forward(octree_in = self.batch['octree_in_partial'], evaluate=True)
        filename = self.batch['filename'][0]
        pos = filename.rfind('.')
        if pos != -1: 
            filename = filename[:pos]  # remove the suffix
        ...
        os.makedirs(save_dir, exist_ok=True)
        bbox = self.batch['bbox'][0].numpy() if 'bbox' in self.batch else None
        self.get_sdfs(output['neural_mpu'], self.batch_size, bbox)  # output['neural_mpu'] is a function.
        self.export_mesh(save_dir, index = 0)

        # save the input point cloud
        pointcloud = trimesh.PointCloud(vertices=self.batch['partial_points'][0].cpu().points.numpy()) # these points are from pointcloud.npz, so the points that are sampled from the mesh in the preprocessing, not the points of the SDF values.
        pointcloud.export(os.path.join(save_dir, 'input_partial.ply'))
