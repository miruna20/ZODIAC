...
from utils.util import VIT_FEATURE_CHANNEL
...
class UNet3DModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                CrossAttention(feature_dim=dim_out, sketch_dim=image_condition_dim,
                               ...
                               drop_out=dropout) if use_cross and use_sketch_condition else our_Identity(),
                nn.Sequential(
        ...
        self.mid_cross_attn = CrossAttention(feature_dim=mid_dim, sketch_dim=image_condition_dim, vit_local=vit_local, vit_global=vit_global,
                                             ...
                                             num_heads=num_heads, image_size=res, world_dims=world_dims,
                                             drop_out=dropout) if use_sketch_condition else our_Identity()
    ...
    def forward_as_middle(self, h, doctree, timesteps, label, context, img_condition=None, projection_matrix=None, kernel_size=None):
        h_lr = octree2voxel(data=h, octree=doctree.octree, depth=self.full_depth)
        h_lr = h_lr.permute(0, 4, 1, 2, 3).contiguous()
        h_lr = self.forward(x=h_lr, timesteps=timesteps, label=label, context=context, as_middle=True, img_condition=img_condition, projection_matrix=projection_matrix, kernel_size=kernel_size)
        x, y, z, b = doctree.octree.xyzb(self.full_depth)
        h_lr = h_lr.permute(0, 2, 3, 4, 1).contiguous()
        h_lr = h_lr[b, x, y, z, :]
        return h_lr
    
    def forward(self, x=None, timesteps=None, x_self_cond=None, label = None, context=None, as_middle=False, img_condition=None, projection_matrix=None, kernel_size=None, **kwargs):
        """
        ...
        x = self.mid_cross_attn(x, img_condition, projection_matrix, kernel_size)
