...
class GraphVAE(torch.nn.Module):
    ...
    def octree_encoder_step(self, octree, doctree):
        ...
        for i, d in enumerate(range(depth, depth_stop-1, -1)): # The encoder operates from depth down to depth_stop
        # perform graph conv
    ...
    def octree_encoder(self, octree, doctree): # The encoder operates from the depth to the full-depth level, which in this case means from 6 to 2.
        convs = self.octree_encoder_step(octree, doctree) # The number of channels in the convolution layers changes with the octree depth from 6 to 2 as follows: [32, 64, 128, 256, 512].
    ...
    def forward(self, octree_in, octree_out=None, pos=None, evaluate=False): # The size of pos here is [batch_size * 5000, 4], which means all the points from every batch are concatenated together, and the last dimension of size 4 is used to represent batch_idx
        # generate dual octrees
        doctree_in = dual_octree.DualOctree(octree_in) # here we transform the octree into a dual octree. See original paper
        ...
        z = posterior.sample() # so we have here a VAE. z are the latents in the octree
        ...
        output['neural_mpu'] = _neural_mpu  # This output['neural_mpu'] is mainly used in the testing phase to return the SDF value for any input pos based on the final layer's reg_voxs.
    ...
    def extract_code(self, octree_in):
        ...
        convs = self.octree_encoder_step(octree_in, doctree_in) # The conv channels change with the octree depth from 6 to 2 as [32, 64, 128, 256, 512]
        code = self.KL_conv(convs[self.depth_stop])
        posterior = DiagonalGaussianDistribution(code)
