...
from utils.util import get_voxel_coordinates, VIT_PATCH_NUMBER, VIEW_IMAGE_RES
import numpy as np
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, sketch_feature, projection_matrix=None, kernel_size=None):
        q = self.q(x).reshape(x.shape[0], self.feature_dim, -1).transpose(1, 2) + self.voxel_pe.to(x.device).unsqueeze(0)
        if self.vit_local and not self.vit_global:
            if projection_matrix is not None:
                voxel_points = torch.from_numpy(get_voxel_coordinates(resolution=x.shape[-1], size=1)).unsqueeze(2).to(projection_matrix.device)
        ...
        k = self.k(sketch_feature) + self.condition_pe.to(x.device).unsqueeze(0)
        v = self.v(sketch_feature) + self.condition_pe.to(x.device).unsqueeze(0)
